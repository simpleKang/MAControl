...
class SimFamily():
    ...
    def update_script_global_params(self):
        ...
        for event_elem in self.script.xmlroot.findall(".//event/condition/condition[@logic='AND']"):
            event_elem.text = 'guidance/field-wp-distance le ' + str(
                self.value_stack['ext/initial-distance-to-field-trigger-ft'][0])
        self.script.xmltree.write(self.script.path)
    ...
    def run_i_sim(self, i, log='off', system='linux'):
        ...
        if log == 'on':
            print(threading.currentThread().getName(),
                  'starting at ' + time.strftime("%H:%M:%S", time.gmtime()) + '. Current number of active threads: ',
        ...
        if log == 'on':
            print(threading.currentThread().getName(),
                  'exiting at ' + time.strftime("%H:%M:%S", time.gmtime()) + '.  Current number of active threads: ',
...
class WRSimFamily(SimFamily):
    ...
    def launch_mc_sim(self, log='off', system='linux'):
        ...
        if system == 'linux':
            os.system(
                cmd_linux + '--catalog=' + self.script.acname + ' > ./JSBSim/catalog_' + self.script.acname + '.txt')
        elif system == 'windows':
            os.system(
                cmd_windows + '--catalog=' + self.script.acname + ' > ./JSBSim/catalog_' + self.script.acname + '.txt')
        ...
        for i in range(1, self.N + 1):
            ...
            ac_config_folderpath = './JSBSim/aircraft/' + self.script.acname + '_port{:}'.format(
                self.port) + idx_str + '/'
...
class PostData():
    ...
    def get_traj_in_NEA(self, lat0=None, lon0=None, flat=True):
        ...
        subP = {key: self.P[key] for key in
                ('Xnea_m', 'Ynea_m', 'Znea_m', 'Xnea_ft', 'Ynea_ft', 'Znea_ft', 's_traj_m', 's_traj_ft')}
...
class PostGroup():
    ...
    def plot_traj3D_UTM(self, origin=[], Hdisk=120, ax_lim=None, nticks=[11, 11, 11], view=(45, -45), to_scale='XYZ',
                        mrk_size=80, legend='off'):
    ...
    def results(self, XY_turb, D_turb, windDir, t_lim=[None, None], linestyle='-'):
        ...
        print('Index ', 'Altit.', 'dist/D', 'nz_max', 'nz_min', 'nz_avg', 'nz_std', 'Ww_max', 'Ww_min', 'Ww_avg',
              'Ww_std', sep='    ')
        print(
            '---------------------------------------------------------------------------------------------------------------')
        ...
        for i in sorted(self.group):
            ...
            D['h_cross'].append(h);
            D['d_cross'].append(d)
            D['nz_max'].append(max(nz));
            D['nz_min'].append(min(nz))
            D['nz_avg'].append(np.mean(nz));
            D['nz_std'].append(np.std(nz))
            D['Vwz_max'].append(max(Vwz));
            D['Vwz_min'].append(min(Vwz))
            D['Vwz_avg'].append(np.mean(Vwz));
            D['Vwz_std'].append(np.std(Vwz))

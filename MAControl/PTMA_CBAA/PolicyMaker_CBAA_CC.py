...
class PolicyMaker_Auction(PolicyMaker):
    ...
    def __init__(self, name, env, world, agent_index, arglist):
        ...
        self.memory_friendID = [[] for t in range(len(world.targets))]
        self.probability = [0.3015, 0.3355, 0.3565, 0.3711, 0.3820, 0.3905, 0.3974, 0.4031]
        self.targetbid_last_time = []
    ...
    def make_policy(self, WorldTarget, obs_n, step, NewController):
        ...
        self.targetbid_last_time = self.targetbid.copy()
                    ...
                    for i in range(len(self.self_bid)):
                        ...
                        if self.targetbid[self.self_bid[i][0]]:
                            ...
                            if not kkk and self.self_bid[i][1]>0:
                                ...
                                break
                            elif kkk and self.self_task[i]:
                                # with open(os.path.dirname(__file__) + '/check.txt', 'a') as f:
                                #     f.write('biding:' + str(min(kkk, key=lambda kkk: kkk[1])) + '\n')
                                ...
                                elif signal == 666 and self.self_task[self.self_bid[i][0]][0] == 1:
                                    # with open(os.path.dirname(__file__) + '/check.txt', 'a') as f:
                                    #     f.write('>>>>>>>>>>>>>>>>>>>>>>>>>>' + '\n')
                                    list_temporary = []
                                    list_temporary_1 = []
                                    signal_temporary = 0
                                    list_temporary_1 = self.targetbid[self.self_bid[i][0]].copy()
                                    list_temporary_1.remove(list_temporary_1[0])
                                    self.targetbid_last_time[self.self_bid[i][0]].remove(self.targetbid_last_time[self.self_bid[i][0]][0])
                                    # 将上一步中的目标报价与这步中的目标报价进行比较，获得其中新添加的个体ID
                                    for bid_one in list_temporary_1:
                                        if bid_one[1] != 0 and bid_one not in self.targetbid_last_time[self.self_bid[i][0]]:
                                            list_temporary.append(bid_one[0])
                                    # 当前目标报价的个体ID是否在记忆中覆盖的友方ID中
                                    for bid_two in list_temporary_1:
                                        if bid_two[0] in self.memory_friendID[self.self_bid[i][0]]:
                                            signal_temporary = 1

                                    if signal_temporary == 0:
                                        for cycle in range(len(list_temporary)):
                                            if random.randint(0, 1) < self.probability[(len(obs_n)*2-20)%10]:
                                                for j in range(len(self.self_task)):
                                                    if self.self_task[j]:
                                                        self.self_task[j][0] = 0
                                                self.self_task[self.self_bid[i][0]][0] = 1

...
class PathPlanner_generate_at_present(PathPlanner):
    ...
    def __init__(self, name, env, world, agent_index, arglist):
        ...
        PathPlanner_generate_at_present.AGENT_ALIVE.append(True)
    ...
    def planpath(self, para_list, obs, arrive_flag, step, worldtarget):
        if para_list[0] == 0:
            ...
            if self.is_attacking is False:
                self.attacking_target = para_list[1][1]
                self.attack_delay(self.attacking_target[0:2])
                self.pointAi = (obs[2], obs[3])
                self.pointBi = (self.waypoint_list[self.current_wplist][0],
                                self.waypoint_list[self.current_wplist][1])
        ...
        if arrive_flag and self.is_attacking is False and self.finished is False:
            ...
            if self.current_wplist > self.arrivals_maxium:
                self.finished = True
                PathPlanner_generate_at_present.AGENT_ALIVE[self.index] = False
    ...
    # 操作数 = 9 根据速度方向进行延长，取与边界交点，同时将目标坐标作为最终点 (并不是)
    def attack_delay(self, coord):

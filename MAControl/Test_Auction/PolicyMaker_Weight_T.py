...
from MAControl.Test_Auction.PathPlanner_generate_at_present import PathPlanner_generate_at_present as PP
...
class PolicyMaker_Weight_T(PolicyMaker):

    def __init__(self, name, env, world, agent_index, arglist):
        super(PolicyMaker_Weight_T, self).__init__(name, env, world, agent_index, arglist)

        self.opt_index = 0                    # 操作数, 目前有 0 无操作 / 1 决策操作 / 9 攻击操作
        self.decision = 0                     # 操作内容

        self.friends_in_sight = list()        # 决策前存储视野中友军
        self.target_to_attack = list()        # 存储将要攻击的目标
        ...
        self.InAttacking = False              # 是否处于攻击状态

        self.num = env.n - len(world.movable_targets)   # 小瓜子数量
        self.w = [0.5, 0, 0.5]           # 权重 w1 / w2 / w3
    ...
    def make_policy(self, WorldTarget, obs_n, step):
        ...
        if self.InAttacking:
            ...
            # if self.after_decision_step < 0:
            #
            #     v2 = self.v_2(obs_n[self.index], self.target_to_attack)
            #     v1 = 0
            #     v3 = 0
            #     v = self.w[0] * v1 + self.w[1] * v2 + self.w[2] * v3
            self.opt_index = 0
            #     self.decision = [v, self.target_to_attack[0:2]]
            ...
            if self.current_targets:
                ...
                if len(self.current_targets) > 1:
                    self.target_to_attack.append(random.choice(self.current_targets))
                else:
                    self.target_to_attack = self.current_targets.copy()
                ...
                v = self.w[0]*v1 + self.w[1]*v2 + self.w[2]*v3
                self.opt_index = 9
                self.decision = [v, self.target_to_attack[0]]
